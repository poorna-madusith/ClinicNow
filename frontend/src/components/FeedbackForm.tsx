      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          {questions.map((item, index) => (
            <div
              key={item.key}
              className="bg-gray-50 p-5 rounded-lg border border-gray-200 hover:border-teal-300 transition-all duration-200"
            >
                ...
                {[1, 2, 3, 4, 5].map((star) => {
                  ...
                  return (
                    <button
                      ...
                      className="transition-all duration-200 transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-offset-2 rounded-full p-1"
        ...
        <DialogFooter className="gap-2">
          <button
            onClick={handleClose}
            disabled={isSubmitting}
            className="px-6 py-2.5 border-2 border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Cancel
          </button>
          <button
            ...
            disabled={isSubmitting}
            className="px-6 py-2.5 bg-gradient-to-r from-teal-500 to-cyan-600 text-white font-semibold rounded-lg hover:from-teal-600 hover:to-cyan-700 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {isSubmitting ? (
              <>
